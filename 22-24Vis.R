...
#####ordering, Averages, scaling up coounts to meter squared------
...
oys_jit = counts %>% 
  ...
  geom_point(data = Rmeans_oys, aes(x = Region, y = mean, size = 4)) +
  ...
  labs(x = 'Region', y = bquote(Oyster~Density~(per~m^2))) + 
...
clus_jit = reef %>% 
  ...
  geom_point(data = Rmeans_clusters, aes(x= Region, y = mean, size = 4, shape = 1, color = 'black')) +
  scale_color_manual(values = sitecolours) +
...
Percent_bar = ggplot(percents, aes(fill=CoverType, y=Percent, x=Region)) + 
  ...
  theme(axis.text = element_text(color = "black"),
        ...
        plot.title = element_text(hjust = 0.5)) + labs(x = 'Region', y = 'Average Percent Cover', fill = "Cover Type") + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +  scale_fill_manual(values = cover_colors)
